...
MAX_READ_FAIL = 10        # 连续读取失败多少次认为断流
...
# RTSP 可用性检测（关键）
# =========================
def check_rtsp_available(rtsp_url, timeout=5):
...
# 安全打开 RTSP
# =========================
def safe_open_capture(rtsp_url, timeout=3):
    result = {}
...
# 启动流（带预检测）
# =========================
def start_stream(camera_id, rtsp_url, ip, algorithmtypes):

    # ===== 坏流冷却 =====
    if camera_id in bad_cameras:
        if time.time() - bad_cameras[camera_id] < BAD_TIMEOUT:
            print(f"[SKIP] 坏流冷却中: {camera_id}")
            return
        else:
            del bad_cameras[camera_id]

    # ===== 启动前检测（关键）=====
    if not check_rtsp_available(rtsp_url):
        print(f"[ERROR] RTSP不可用: {camera_id}")
    ...
    t = threading.Thread(
        ...
        args=(camera_id, rtsp_url, ip, algorithmtypes, stop_event),
        daemon=True
    )
    t.start()
...
# 上传视频
def upload_warning_video(video_bytes, camera_id, now):
    # timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    object_name = f"{camera_id}/{now}.mp4"
...
# 生成报警视频
def generate_warning_video_memory(frame_buffer, video_frames, FPS, camera_id, now):
    try:
        ...
        # === 上传到 MinIO ===
        video_url = upload_warning_video(video_bytes, camera_id, now)
...
# 异步生成预警视频
def _async_generate_video_and_notify(pre_frames, rtsp_url, FPS, camera_id, image_url, now):
    try:
        post_frames = record_post_frames(rtsp_url, seconds=3)
        video_url = generate_warning_video_memory(pre_frames, post_frames, FPS, camera_id, now)
...
def _async_generate_video_and_notify2(pre_frames, rtsp_url, FPS, camera_id, image_url, now):
    try:
        post_frames = record_post_frames(rtsp_url, seconds=3)
        video_url = generate_warning_video_memory(pre_frames, post_frames, FPS, camera_id, now)
...
# 供人员聚集使用
def _async_generate_video_and_notify3(pre_frames, rtsp_url, FPS, camera_id, image_url, now):
    try:
        post_frames = record_post_frames(rtsp_url, seconds=3)
        video_url = generate_warning_video_memory(pre_frames, post_frames, FPS, camera_id, now)
        # batch = []
        ...
        send_alarm_to_java(update, config["server_endpoints3"])
    except Exception as e:
        print(f"[ERROR] 异步生成预警视频失败: {e}", flush=True)
...
def alarm_dispatcher_loop(config, alarm_queue):
    while True:
        alarm_dispatcher(config, alarm_queue)

def safehat_dispatcher_loop(config, alarm_queue):
    while True:
        safehat_dispatcher(config, alarm_queue)

def personcrowd_dispatcher_loop(config, queue):
    while True:
        try:
            ...
            print(f"[ERROR] 聚集报警发送异常: {e}", flush=True)

def alarm_dispatcher(config, alarm_queue):
    try:
        data = alarm_queue.get(timeout=1)
    except queue.Empty:
        return
    # 包装为数组
    batch = [data]
...
def safehat_dispatcher(config, safehat_queue):
    try:
        data = safehat_queue.get(timeout=1)
    except queue.Empty:
        return
    # 包装为数组
    batch = [data]
...
def detect_intrusion(frame, camera_id, rtsp_url, config, frame_buffer, FPS, ip_address):
    ...
    for det in detections:
        ...
        now1 = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        if not state["active"]:
            ...
            try:
                ...
                threading.Thread(
                    ...
                    args=(pre_frames, rtsp_url, FPS, camera_id, image_url, now1),
                    daemon=True
                ).start()
            except Exception as e:
        ...
        detected_objects = [{
            ...
            "warning_video": f"http://172.21.3.141:8084/yolo/{camera_id}/{now1}.mp4"
...
def check_helmet_and_alert(frame, camera_id, rtsp_url, config, frame_buffer, FPS, ip_address):
    ...
    for det in detections:
        ...
        if not state["active"]:
            ...
            threading.Thread(
                target=safehat_dispatcher_loop,
...
def detect_overcrowding(frame, camera_id, rtsp_url, config, frame_buffer, FPS, ip_address):
    ...
    for det in detections:
        ...
        for (rx1, ry1, rx2, ry2, conf, cls_id, label) in valid_dets:
            color = (0, 255, 255)
            cv2.rectangle(frame_draw, (rx1, ry1), (rx2, ry2), color, 2)
            cv2.putText(frame_draw, f"{label} {conf:.2f}",
                        (rx1, max(ry1 - 8, 0)),
        ...
        # ===== 事件开始：上传图片 =====
        try:
            image_url = upload_warning_image(frame_draw, camera_id, "overcrowding")
        except Exception as e:
        ...
        try:
            ...
            threading.Thread(
                ...
                args=(pre_frames, rtsp_url, FPS, camera_id, personcrowd_queue_video, image_url, config, now),
                daemon=True
            ).start()
        except Exception as e:
        ...
        personcrowd_queue.put({
            ...
            "warning_image": image_url,
            "warning_video": f"http://172.21.3.141:8084/yolo/{camera_id}/{now}.mp4",
            "ip": str(ip_address)
        })

        threading.Thread(
            target=personcrowd_dispatcher_loop,
            args=(config, personcrowd_queue),
            daemon=True
        ).start()
...
def process_stream(camera_id, rtsp_url, ip_address, algorithmtypes, stop_event):
    ...
    while not stop_event.is_set():
        ...
        if cap is None or not cap.isOpened():
            ...
            if reconnect_count >= MAX_RECONNECT:
                print(f"[FATAL] 摄像头 {camera_id} 重连超过 {MAX_RECONNECT} 次，停止线程")
                bad_cameras[camera_id] = time.time()
                break  # 直接退出线程
        ...
        if not ret:
            print(f"[WARN] 摄像头 {camera_id} 读取失败")
            cap.release()
            cap = None
            continue
        ...
        if "1" in parts and "2" in parts:
            # print("同时包含 1 和 2")
            intrusion_active, _ = detect_intrusion(frame, camera_id, rtsp_url, config, frame_buffer, FPS,
                                                   ip_address)
...
def camera_poll_loop(cams, interval=600):
    while True:
        try:
            print(f"发现 {len(cams)} 个有效摄像头")
            ...
            for cam_id, cam in new_map.items():
                ...
                cam_id, rtsp_url, ip_address, algorithmtypes = cam

                if cam_id not in RUNNING_STREAMS:
                    start_stream(cam_id, rtsp_url, ip_address, algorithmtypes)
...
if __name__ == "__main__":
    ...
    # threading.Thread(target=camera_monitor_loop, daemon=True).start()
    #
