from flask import Flask, request, jsonify
from flask_cors import CORS
from multiprocessing import Process
import threading
from collections import defaultdict
from push import stop_stream, stream_worker3
import cv2

"""
常量区
"""
# 电子围栏区域（动态更新）
fence_dict = {}
algorithm_dict = {}

fence_lock = threading.Lock()
algorithm_lock = threading.Lock()

# 存储当前运行的摄像头线程 & process
active_streams = {}
stream_stop_flags = {}

# 导入模型
# model = YOLO('./models/yolov8n.pt')

# 全局缓存（按摄像头）
latest_pre_frames = defaultdict(list)

# 解析坐标，获取矩形框
def rect_to_polygon(rect, frame_width, frame_height):

    base_area = {"width": 960, "height": 540}

    # 按实际视频大小映射到逻辑区域
    scale_x = frame_width / base_area["width"]
    scale_y = frame_height / base_area["height"]

    # 映射后的实际视频坐标
    x = rect["x"] * scale_x
    y = rect["y"] * scale_y
    w = rect["width"] * scale_x
    h = rect["height"] * scale_y

    return [
        [int(x), int(y)],  # 左上
        [int(x + w), int(y)],  # 右上
        [int(x + w), int(y + h)],  # 右下
        [int(x), int(y + h)]  # 左下
    ]

"""
启动服务
"""

app = Flask(__name__)
CORS(app)
lock = threading.Lock()
# config = load_config()
# cams = get_cameras()
# start_all_streams(config, fence_lock, algorithm_lock, fence_dict, algorithm_dict, latest_pre_frames, cams)


import subprocess

def get_rtsp_resolution(rtsp_url):
    cmd = [
        "ffprobe",
        "-v", "error",
        "-select_streams", "v:0",
        "-show_entries", "stream=width,height",
        "-of", "csv=p=0",
        rtsp_url
    ]
    try:
        result = subprocess.check_output(cmd, timeout=5).decode().strip()
        width, height = map(int, result.split(','))
        return width, height
    except Exception as e:
        print(f"[ERROR] ffprobe 获取分辨率失败: {e}")
        return None, None


"""
API 接口
"""
# 根路由
@app.route("/")
def index():
    return "Service is running", 200

# 接收围栏设置请求
@app.route("/set_fence", methods=["POST"])
def set_fence():
    data = request.get_json() or {}
    algorithmType = data.get("algorithmType")
    rtsp_url = data.get("url")
    rect = data.get("fence_area")
    default_area = data.get("default_area")
    camera_id = data.get("cam_id")

    cap = cv2.VideoCapture(rtsp_url)
    # cap 判断是否打开成功

    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    # width, height = get_rtsp_resolution(rtsp_url)

    if not width or not height:
        return jsonify({"status": "error", "message": "无法获取分辨率"}), 500

    fps = 25

    print(f"[DEBUG] 摄像头分辨率: {width}x{height}", flush=True)

    try:
        if rect:
            fence_area = rect_to_polygon(rect, width, height)
    except Exception as e:
        return jsonify({"status": "error", "message": f"fence_area 格式错误: {e}"}), 400

    print(f"[INFO] 请求启动 摄像头={camera_id} rtsp={rtsp_url} fence={fence_area} algorithmType={algorithmType} default_area={default_area}", flush=True)

    # ==== 若已存在旧流，先彻底停止 ====
    if camera_id in active_streams:
        print(f"[INFO] 摄像头 {camera_id} 已在运行，尝试停止旧流")
        try:
            stop_stream(camera_id, fence_dict)  # 停止旧线程
            del active_streams[camera_id]
        except Exception as e:
            print(f"[WARN] 停止旧流失败: {e}")

    # ==== 启动独立进程运行 FFmpeg ====
    p = Process(target=stream_worker3, args=(camera_id, rtsp_url, fence_area))
    p.daemon = True
    p.start()
    active_streams[camera_id] = p
    # print(f"[DEBUG] 已启动 FFmpeg 进程 ........................................", flush=True)

    # ==== 设置围栏 ====
    with fence_lock:
        fence_dict[camera_id] = fence_area

    with algorithm_lock:
        algorithm_dict[int(camera_id)] = algorithmType

    output_url = f"rtsp://10.168.60.52:8553/Streaming/Channels/{camera_id}"
    print(f"[DEBUG] urllllllllllllllll", output_url, flush=True)

    return jsonify({
        "status": "success",
        "camera_id": camera_id,
        "output_url": output_url
    })

@app.route("/delete_stream", methods=["POST"])
def delete_stream():
    data = request.get_json() or {}
    camera_id = data.get("cam_id")

    if not camera_id:
        return jsonify({"status": "error", "message": "cam_id required"}), 400

    if camera_id in active_streams:
        try:
            stop_stream(camera_id, fence_dict)
            del active_streams[camera_id]
            print(f"[INFO] 摄像头 {camera_id} 已停止", flush=True)
        except Exception as e:
            return jsonify({"status": "error", "message": str(e)}), 500
    else:
        return jsonify({"status": "warning", "message": "camera not running"}), 200

    return jsonify({"status": "success", "camera_id": camera_id})

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5005, debug=False)
